...
keybinding-desc-workspace-left-panel-git-history = 左侧面板：源代码管理
...
workspace-left-panel-git-history = 源代码管理
git-working-tree-title = 更改
git-working-tree-staged = 已暂存的更改
git-working-tree-unstaged = 更改
git-working-tree-loading = 正在加载工作区更改…
git-working-tree-load-error = 工作区更改加载失败
git-working-tree-clean = 当前没有工作区更改。
git-working-tree-diff-title = 工作区 Diff
git-sidebar-show-history = 历史
git-sidebar-show-changes = 更改
git-history-title = Git 提交历史
git-history-refresh = 刷新 Git
git-history-loading = 正在加载 Git 提交历史…
git-history-loading-files = 正在加载变更文件…
git-history-load-error = Git 提交历史加载失败
git-history-empty = 当前分支没有提交记录。
git-history-no-local-repository = 当前没有聚焦的本地 Git 仓库。第一版暂不支持远程和 SSH 仓库。
git-history-no-changed-files = 此提交没有记录变更文件。
git-history-load-more = 加载更多
git-history-diff-title = Git 历史 Diff
git-history-diff-empty-selection = 请从左侧选择一个变更文件。
git-history-diff-loading = 正在加载历史 Diff
git-history-diff-load-error = Diff 加载失败
git-history-diff-binary = 这是二进制文件，无法显示文本 Diff。
git-history-diff-too-large = 此文件的 Diff 过大，暂不显示。
git-history-diff-hidden-bidi = 此 Diff 包含隐藏的双向控制字符，请谨慎检查原始内容。
git-history-diff-no-text-changes = 此文件没有文本内容变化。
git-history-files-load-error = 变更文件加载失败
